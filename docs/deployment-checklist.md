...
- Supabase → **Authentication → Passwords**에서 최소 8자 이상 설정 확인
- 앱 회원가입 기준과 동일하게 비밀번호는 영문+숫자 포함으로 안내
- 외부 OAuth(카카오 등) 설정 **불필요** — 학번 로그인으로 자체 완결
- (가입 흐름) 앱에서 `학번@ai-campus.local`로 signUp → `profiles`에 학번·이름·과 저장
- `profiles` 원본 조회는 본인/admin만 허용하고, 공개 작성자 표시는 `public_profiles` 뷰만 사용
...
- `NEXT_PUBLIC_SITE_URL`은 Vercel 배포 URL 또는 커스텀 도메인으로 설정
- `service_role` 키는 Vercel/클라이언트 환경변수에 등록하지 않음
